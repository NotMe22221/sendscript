...
export async function POST(request: Request) {
  try {
    ...
    const requirements = openai ? await extractMission(prompt, openai.credentials) : demoRequirements;
    let missionId = DEMO_MISSION_ID;
    ...
    return ok({ missionId, requirements }, openai && context.mode === "live" ? "live" : "demo", 201);
