...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (context.mode === "live" && context.supabase) {
      ...
      if (input.amountCents !== 35_800 || input.amountCents <= Number(authorization.amount_cap_cents)) {
        throw new RouteError("INVALID_VIOLATION_PROOF", "The judge guardrail proof must test the $358 charge shown in Mission Control and exceed the approved cap.", 400, false);
      }
    ...
    if (blocked) await appendActivityEvent(context, input.missionId, "transaction.blocked", "Out-of-policy charge blocked", `${(input.amountCents / 100).toFixed(2)} USD exceeded the approved mandate cap. No checkout ran and no funds moved.`, resolved.live ? "Prava sandbox" : "Prava simulation");
    return ok(result, resolved.live && context.mode === "live" ? "live" : "demo");
